...
def list_monthly_budgets(
    year: int | None = Annotated[Query(None)],
    month: int | None = Annotated[Query(None)],
...
def get_monthly_budget(budget_id: int, db: Session = Annotated[Depends(get_db)]):
...
def create_monthly_budget(data: MonthlyBudgetCreate, db: Session = Annotated[Depends(get_db)]):
    return svc.create_monthly_budget(db, data)


@router.patch("/{budget_id}", response_model=MonthlyBudget)
def update_monthly_budget(budget_id: int, data: MonthlyBudgetUpdate, db: Session = Annotated[Depends(get_db)]):
...
def delete_monthly_budget(budget_id: int, db: Session = Annotated[Depends(get_db)]):
